...
class CreateAccountForm extends FormBase
{
    ...
    {
        return 'zoho_custom_campaign_create_account_form';
    }
    ...
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        ...
        $form['accountname'] = [
            ...
            '#required' => TRUE,
            '#description' => $this->t('Enter a name for your account.'),
        ];
    ...
    public function submitForm(array &$form, FormStateInterface $form_state)
    {
        ...
        if ($result && (isset($result['campaignKey']) || isset($result['campaign_key']))) {
            $this->messenger()->addStatus($this->t('Account "@name" has been successfully created.', [
                '@name' => $campaignData['campaignname'],
            ]));
        } else {
            $this->messenger()->addError($this->t('Failed to Account campaign. Please check the logs.'));
        }
